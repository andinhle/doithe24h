...
{
  $result = mysqli_query($ketnoi,"SELECT * FROM `account` WHERE `username` LIKE '%".$_POST['timkiem']."%' ORDER BY id desc limit 0, 1000");
}
else
{
  $result = mysqli_query($ketnoi,"SELECT * FROM `account` ORDER BY id desc limit 0, 100000");
}
?>
<?php
...
<?php
    ...
    if (isset($_POST["btn_submit"]))
    {
        ...
        $query_user_dangky = mysqli_query($ketnoi,"SELECT * FROM `account` WHERE `username` = '".$username."' ");
        $query_email_dangky = mysqli_query($ketnoi,"SELECT * FROM `account` WHERE `mail` = '".$username."' ");
        ...
        {
            $create = mysqli_query($ketnoi,"INSERT INTO `account` SET 
            `username` = '".$username."',
            `password` = '".$password."',
            `mail` = '".$mail."',
            `createdate` = now() ");
